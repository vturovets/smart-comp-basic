...
def run_unimodality_analysis(cleaned_file, config, logger=None):
    ...
    x_grid = np.linspace(data.min(), data.max(), 2000)  # finer resolution
    kde_values = kde(x_grid)
    kde_values = pd.Series(kde_values).rolling(window=3, center=True, min_periods=1).mean().values  # less smoothing

    max_kde = np.max(kde_values)
    peak_prominence = 0.0025 * max_kde  # more sensitive

    main_peak_index = np.argmax(kde_values)
    peaks, properties = find_peaks(kde_values, prominence=peak_prominence, distance=25)  # avoid spurious tail peaks
    all_peaks = set(peaks)
