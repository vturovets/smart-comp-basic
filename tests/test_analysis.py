...
def test_run_descriptive_analysis_basic(dummy_config):
    ...
    result = run_descriptive_analysis(df, dummy_config)
...
def test_run_descriptive_analysis_small_sample(dummy_config):
    df = pd.DataFrame(np.random.normal(0, 1, 5))
    result = run_descriptive_analysis(df, dummy_config)
    assert 'mean' in result


def test_check_unimodality_kde_normal(dummy_config):
    df = pd.DataFrame(np.random.normal(0, 1, 1000))
    assert check_unimodality_kde(df, dummy_config) is True


def test_check_unimodality_kde_multimodal(dummy_config):
    df1 = np.random.normal(0, 1, 500)
    df2 = np.random.normal(5, 1, 500)
    df = pd.DataFrame(np.concatenate([df1, df2]))
    assert check_unimodality_kde(df, dummy_config) is False
